...
for i,row in enumerate(row_list):
  ...
  for j,number in enumerate(row):
    if((highest is None) or (number > highest)): highest = number
    if((lowest is None) or (number < lowest)): lowest = number
  count = count + highest - lowest
...
for i,row in enumerate(row_list):
  for j,number in enumerate(row):
    for k, compareNumber in enumerate(row):
      if((number % compareNumber == 0) and (number != compareNumber)): 
        count2 = count2 + (number / compareNumber)
